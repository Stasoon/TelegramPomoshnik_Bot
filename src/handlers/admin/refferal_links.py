...
class Handlers:
    ...
    async def __handle_delete_link_callback(callback: CallbackQuery, state: FSMContext):
        await callback.message.edit_text(
            '🔘 Введите название ссылки, которую хотите удалить. ', reply_markup=Keyboards.cancel_markup
        )
    ...
    async def __handle_links_list(callback: CallbackQuery):
        ...
        await callback.message.edit_text(text, reply_markup=Keyboards.cancel_markup)

    @staticmethod
    async def __handle_find_link_callback(callback: CallbackQuery, state: FSMContext):
        await callback.message.edit_text('🔎 Введите название ссылки:', reply_markup=Keyboards.cancel_markup)
    ...
    async def __handle_cancel_callback(callback: CallbackQuery, state: FSMContext):
        await callback.message.edit_text('Что вы хотите сделать?', reply_markup=Keyboards.reflinks_markup)
        await state.finish()
